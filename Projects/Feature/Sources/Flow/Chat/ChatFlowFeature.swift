...
/// 대화 탭의 화면 스택. 자리표시 화면이 root 이고 그 위로 쌓이는 화면은 아직 없다.
///
/// 화면을 그리지 않는다. 경로와 자식만 갖는다
@Reducer
public struct ChatFlowFeature {
    /// 대화(root) 위로 쌓이는 화면. 아직 없다
    ...
    public struct State: Equatable {
        ...
        public init(
            placeholder: PlaceholderFeature.State = PlaceholderFeature.State(title: "Chat"),
            path: [Route] = []
        ) {
