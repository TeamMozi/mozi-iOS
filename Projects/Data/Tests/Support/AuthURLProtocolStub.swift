...
final class AuthURLProtocolStub: URLProtocol, @unchecked Sendable {
    ...
    override func startLoading() {
        ...
        do {
            guard let handler = Self.requestHandler else {
                throw NSError(domain: "AuthURLProtocolStub", code: 1)
            }
            let stub = try handler(observedRequest)
            guard let url = observedRequest.url ?? URL(string: "https://example.invalid") else {
                throw NSError(domain: "AuthURLProtocolStub", code: 2)
            }
            guard let response = HTTPURLResponse(
                ...
                throw NSError(domain: "AuthURLProtocolStub", code: 3)
            }
